...
**Stage A geometry-free** (`.github/workflows/stage-a-no-geometry.yml`):
- Runs on PR and on push to `main`/`master`. Stdlib-only, no pip install, so
  there is nothing to pin and nothing to drift
- Scans `vop_interwoven/` — where the Stage A runtime lives. `tools/` is
  excluded because nothing under it runs inside a capture. **Scope is not the
  repo**, and unlike `no-bare-except.yml` that is deliberate here
- A path typo cannot certify an empty scan: the tool prints the file count and
  **refuses** (exit 2, `NOT PROVEN`) on a missing scan root, a zero-file scan,
  an unparseable file, an unresolvable root, or a computed callee inside the
  path. Exit 2 fails the job — it is not a skip
- **The job falsifies itself on every run.** A first step asserting `PROVEN`
  would only ever prove "the tool said PROVEN", and a check that has quietly
  stopped working prints that too, forever — which this repo has shipped more
  than once. So a second step injects a real `get_Geometry` call into a
  function the Stage A roots genuinely reach, in a throwaway copy, and
  **requires exit 1**. If that step passes, the green above it is a
  measurement rather than a decoration
- The injection anchors on a literal line in `color_id_buffer.py`; if that
  line moves, the step fails loudly telling you to update the workflow, rather
  than silently injecting nothing and "passing"
...
# Also enforced in CI by stage-a-no-geometry.yml, which additionally
# falsifies the check against an injected known-positive on every run.
