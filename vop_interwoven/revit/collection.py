...
def _bbox_world_corners(bbox, transform=None, bbox_is_link_space=False, diag=None, view_id=None, elem_id=None):
    """Return a bounding box's 8 corners in final host/world space, after
    applying bbox.Transform (bbox-local -> the space bbox_is_link_space
    describes) and then, when bbox_is_link_space is True, ``transform``
    (link -> host).

    Shared by project_bbox_corners_uv() and project_bbox_uv_and_near_face_w()
    so a UV footprint and its near_face_w are always derived from the
    IDENTICAL transformed corner set -- computing them via two independent
    code paths (one applying bbox.Transform, one not) could otherwise put
    them in different coordinate spaces for an element whose BoundingBoxXYZ
    carries a non-identity Transform (e.g. a rotated family instance),
    silently pairing a correct UV footprint with a near_face_w computed from
    un-transformed, bbox-local corners.

    Returns a list of 8 (x, y, z) tuples, or None if a required transform is
    missing or a Revit API failure prevents completing it.
    """
    if bbox is None:
        return None
    ...
    if bbox_trf is not None:
        try:
            ...
            try:
                ...
                # not the space this function promises to return. Returning
                # them anyway would silently produce plausible-looking but
                # wrong data -- honor the documented "None on failure"
                # contract instead of continuing with untransformed corners.
                return None
...
def project_bbox_uv_and_near_face_w(bbox, vb, transform=None, bbox_is_link_space=False, diag=None, view_id=None, elem_id=None):
    """Project a bounding box to its UV footprint rectangle AND its
    near-face depth (nearest w across the same transformed 8 corners) in
    one pass, guaranteeing both values come from the identical corner set.

    color_id_buffer.py's near-face-W sidecar collection (Phase 1b) uses
    this instead of calling project_bbox_corners_uv() and
    estimate_nearest_depth_from_bbox() separately: that function pair can
    silently disagree on coordinate space, since estimate_nearest_depth_
    from_bbox() never applies bbox.Transform (only the bbox_is_link_space
    ``transform`` parameter) while project_bbox_corners_uv() always does --
    for an element whose BoundingBoxXYZ carries a non-identity Transform
    (e.g. a rotated family instance), that mismatch would pair a correctly
    transformed UV footprint with a near_face_w computed from un-transformed,
    bbox-local corners. Deliberately does not call estimate_nearest_depth_
    from_bbox() at all, to avoid depending on (or risking any behavior
    change to) that shared function, which the core occlusion pipeline
    (render_model_front_to_back) also relies on.

    Args:
        Same as project_bbox_corners_uv().

    Returns:
        (bbox_corners_uv, near_face_w) -- both None if projection cannot be
        completed; otherwise bbox_corners_uv is the same 4-corner UV
        rectangle project_bbox_corners_uv() would return and near_face_w is
        the minimum w (nearest depth) among the 8 transformed corners.
    """
    from .view_basis import world_to_view
    ...
    try:
        uvws = [world_to_view(corner, vb) for corner in corners]
    except Exception as e:
        if diag is not None:
            diag.error(
                phase="collection",
                callsite="project_bbox_uv_and_near_face_w",
                message="Exception in world_to_view: {}".format(e),
                exc=e,
                view_id=view_id,
                elem_id=elem_id,
    ...
    u_min = min(p[0] for p in uvws)
    u_max = max(p[0] for p in uvws)
    v_min = min(p[1] for p in uvws)
    v_max = max(p[1] for p in uvws)
    near_face_w = min(p[2] for p in uvws)
    rect = [[u_min, v_min], [u_max, v_min], [u_max, v_max], [u_min, v_max]]
