...
def export_annotation_color_id_buffer_view(doc, view, cfg, geom, diag=None,
    ...
    # NOTE: the sidecar was already written above, so the PERSISTED file does
    # not carry capture_faults or failure_reason. That is a real defect and it
    # is fixed in its own PR (#216), not here -- this branch is a probe and its
    # analyzer reads faults from the probe's combined record, which comes from
    # the RETURNED metadata below and was never affected. When #216 lands, this
    # comment and the early write above go with it.
