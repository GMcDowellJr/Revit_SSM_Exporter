...
| `vop_interwoven/pipeline.py` | Production view processing, raster initialization, collection/raster coordination | **Current production.** Imported by production entry/streaming and current probes (`init_view_raster`). | `entry_dynamo`, `streaming`, graphics-semantics, external-sources, and model-linework probes. | Revit-dependent processing; no Stage A probe `TransactionGroup` ownership observed. | `Config`, Document/views; pipeline results/metrics. |
...
| `vop_interwoven/revit/collection.py` | Current view collection and HOST/LINK/DWG expansion | **Current production.** Pipeline and four current probes import it. | Pipeline plus graphics-semantics, external-sources, model-linework, and minimum-mutations probes; model-linework uses collection in both its model-category and reference-element collectors. | Revit-dependent; collection does not own probe transaction groups. | View/config/diagnostics; element collections and source entries. |
