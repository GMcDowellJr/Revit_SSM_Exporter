...
def _run_native(raw_view, output_dir, max_elements, selection, resolution_policy=DEFAULT_RESOLUTION_POLICY, target_dpi=DEFAULT_TARGET_DPI, fixed_pixel_width=DEFAULT_FIXED_PIXEL_WIDTH, max_pixel_dimension=DEFAULT_MAX_PIXEL_DIMENSION):
...
def run_probe(raw_view, output_dir, max_elements=None, selection="all",
    ...
    return _probe_contract().execution_envelope(
        PROBE_NAME, {"selection": selection, "max_elements": max_elements,
...
def dynamo_main(inputs):
    return run_probe(inputs[0] if len(inputs) > 0 else None, inputs[1] if len(inputs) > 1 else None, inputs[2] if len(inputs) > 2 else None, inputs[3] if len(inputs) > 3 else "all", inputs[4] if len(inputs) > 4 else DEFAULT_RESOLUTION_POLICY, inputs[5] if len(inputs) > 5 else DEFAULT_TARGET_DPI, inputs[6] if len(inputs) > 6 else DEFAULT_FIXED_PIXEL_WIDTH, inputs[7] if len(inputs) > 7 else DEFAULT_MAX_PIXEL_DIMENSION)
